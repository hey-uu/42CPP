...
bool ClapTrap::_isDead(void) const {
  if (_hitPoints != 0)
    return (false);
  std::cout << ": ClapTrap is dead... (hit points = 0)" << std::endl;
  return (true);
}

bool ClapTrap::_isExhausted(void) const {
  if (_energyPoints != 0)
    return (false);
  std::cout << ": ClapTrap is exhausted... (energy points = 0)" << std::endl;
  return (true);
}