...
Fixed::Fixed(int const intVar) { fixedNum = (intVar << fractionBits); }
...
int Fixed::getRawBits(void) const { return (fixedNum); }

void Fixed::setRawBits(int const raw) { fixedNum = raw; }
...
Fixed Fixed::operator+(Fixed const &ref) const {
  Fixed res;
  res.setRawBits(this->fixedNum + ref.fixedNum);
  return (res);
}
